...
const Statistics = () => {
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      try {
        setIsLoading(true);
        const res = await API.get('/admin/dashboard-stats', { headers: getAdminAuthHeader() });
    ...
    <Box>
      ...
      {isLoading ? (
        <Flex align="center" justify="center" py={16} gap={3}>
          ...
          <Text fontSize="sm" color={emptyColor}>Loading dashboard...</Text>
        </Flex>
      ) : isError ? (
        <Flex direction="column" align="center" justify="center" py={16} gap={2}>
          <Icon as={FiShoppingBag} boxSize={8} color="red.400" />
          <Text fontSize="sm" color="red.500" fontWeight="500">Failed to load dashboard</Text>
          <Text fontSize="xs" color={emptyColor}>Please refresh or try again.</Text>
        </Flex>
      ) : (
        <>
          <Grid
